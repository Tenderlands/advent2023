...
# KTJJT 220
# QQQJA 483"""
listInput = listInput.replace("A", "D").replace("K", "C").replace("Q", "B").replace("J", "1").replace("T", "A").split(
    "\n")
print(listInput)
handDict = {
    "FiveOfAKind": [],
...
def determine_type(hand, bid):
    ...
    for card in hList:
        if card not in cardList.keys():
            ...
            cardList[card] += 1
    numberList = []
    if '1' in cardList.keys() and len(cardList) > 1:
        jokers = cardList['1']
        del cardList['1']
        cardList[max(cardList, key=cardList.get)] += jokers
